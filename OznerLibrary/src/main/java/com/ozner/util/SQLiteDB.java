...
public class SQLiteDB extends SQLiteOpenHelper {
     ...
     * @return
     */
    public boolean checkColumnExists(String columnName,String table) {
        SQLiteDatabase db = this.getWritableDatabase();
        boolean result = false ;
        Cursor cursor = null ;
        try{
            cursor = db.rawQuery( "select * from sqlite_master where name = ? and sql like ?"
                    , new String[]{table , "%" + columnName + "%"} );
    ...
    public void addColumn(String columnName,String table)
    {
        if(columnName!=null&&columnName.length()>0)
        {
            SQLiteDatabase db = this.getReadableDatabase();
            try {
                db.execSQL(String.format("ALTER TABLE %s  ADD COLUMN %s nvarchar(500)", table, ColCUModel));
            }catch (Exception ex)
            {
                ex.printStackTrace();
            }
