 ...
 */
public class MatchCupActivity extends AppCompatActivity {
    ...
    private void initView() {
        ...
        restartSearch.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                ((TextView) findViewById(R.id.matchcup_tv_downside)).setText(getString(R.string.match_glass_notice));
                ((TextView) findViewById(R.id.matchcup_tv_bluetooth)).setText(getString(R.string.matching_bluetooth));
                ((TextView) findViewById(R.id.tv_match_glass_notice2)).setText(getString(R.string.clear_up_water));
                searchingDevice();
            }
        });
...
//                toolbarText.setText(getString(R.string.match_failed));
        ...
        ((TextView) findViewById(R.id.tv_match_glass_notice2)).setText(getString(R.string.problem_notice_glass));
    }
    ...
    class TimerCount extends CountDownTimer {
        ...
        public void onFinish() {
            ...
            if (deviceNum == 0) {
                ShowSearchFailed();
                try {
                    MatchCupActivity.this.unregisterReceiver(mMonitor);
                    mMonitor = null;
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
        ...
        public void onBindViewHolder(ViewHolder convertView, final int position) {
            ...
            if (device != null) {
                if (Mac != null && device.getAddress().equals(Mac)) {
                    convertView.Cup_iv_device_item_image.setImageResource(R.drawable.glass);
                    convertView.item_selected.setChecked(true);
                } else {
                    convertView.Cup_iv_device_item_image.setImageResource(R.drawable.glass_small);
                    convertView.item_selected.setChecked(false);
                }
                convertView.Cup_tv_device_item_name.setText(getString(R.string.smart_glass));
    ...
    private void SaveDevice(BaseDeviceIO deviceIO) {
        ...
        try {
            ...
            if (device != null) {
                ...
                if (weight.isEmpty()) {
                    device.setAppdata(PageState.DEVICE_WEIGHT, "55");
                    device.setAppdata(PageState.DRINK_GOAL, "" + Math.round((55 * 27.428)));
                } else {
                    device.setAppdata(PageState.DEVICE_WEIGHT, weight);
                    int wei = Integer.parseInt(weight);
                    device.setAppdata(PageState.DRINK_GOAL, "" + Math.round((wei * 27.428)));
                }
                device.setAppdata("checkUpdate", "time");//为了判断固件是否需要升级的次数
                //添加网络缓存任务
