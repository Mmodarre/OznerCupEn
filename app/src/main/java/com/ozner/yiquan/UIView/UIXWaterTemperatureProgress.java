...
/**
 * Created by taoran on 15/11/26.
 */
public class UIXWaterTemperatureProgress extends UIXBaseView {
    float _lineWidth = 0;
    float _space;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            tds_layout=ViewReturn.getProgressRelativeLayout();
            tds_layout.setVisibility(View.GONE);
            chart_layout=ViewReturn.getChartRelativeLayout();
    ...
    public UIXWaterTemperatureProgress(Context context, AttributeSet attrs) {
        super(context, attrs);
        mcontext=context;
        init();
    }
    ...
    protected void onDrawText(Canvas canvas) {
        ...
        String text = String.format(getResources().getString(R.string.bad_temperature)+" %d%%", _bad_progress);
        canvas.drawText(text, this.getWidth() / 2 - space, offset, badPaint);
        offset += _space + _lineWidth;

        text = String.format(getResources().getString(R.string.normal_temperature)+" %d%%", _normal_progress);
        canvas.drawText(text, this.getWidth() / 2 - space, offset, normalPaint);

        offset += _space + _lineWidth;
        text = String.format(getResources().getString(R.string.good_temperature)+" %d%%", _good_progress);
