...
import com.ozner.qianye.ACSqlLite.CCacheWorking;
import com.ozner.qianye.AirPurifier.DeskAirPurifierFragment;
import com.ozner.qianye.AirPurifier.VerticalAirPurifierFragment;
import com.ozner.qianye.BaiduPush.OznerBroadcastAction;
import com.ozner.qianye.BaiduPush.PushBroadcastKey;
import com.ozner.qianye.CChat.CChatFragment;
import com.ozner.qianye.CChat.ChatMessageHelper;
...
import com.ozner.qianye.Command.ImageLoaderInit;
import com.ozner.qianye.Command.MsgHandler;
...
import com.ozner.qianye.Device.OznerMallFragment;
...
import com.ozner.qianye.Login.LoginActivity;
import com.ozner.qianye.Main.BaseMainActivity;
import com.ozner.qianye.Main.MainConEnFragment;
import com.ozner.qianye.WaterProbe.WaterCupFragment;
import com.ozner.qianye.WaterProbe.WaterProbeFragment;
import com.ozner.qianye.WaterProbe.WaterPurifierFragment;
import com.ozner.qianye.WaterProbe.WaterReplenishMeterFragment;
import com.ozner.qianye.WaterProbe.WaterTDSPenFragment;
import com.ozner.qianye.mycenter.CenterBean.CenterNotification;
import com.ozner.qianye.mycenter.MyFragment;
import com.ozner.qianye.slideleft.LeftSlideFragment;
...
//        InitFootFragment();//初始化底部导航栏
        ...
        RigisterDevice();
        myHandler = new MsgHandler(MainEnActivity.this);
    }
    ...
    public class CenterVFCheckTask extends AsyncTask<String, Void, List<NetUserVfMessage>> {
        ...
        protected List<NetUserVfMessage> doInBackground(String... params) {
            List<NetUserVfMessage> vfInfoList = OznerCommand.GetUserVerifMessage(MainEnActivity.this);
            return vfInfoList;
        }
...
//        ShowDevicePage(PageState.WODESHEBEI, null);
    }
    ...
    * */
    private void InitFootFragment() {
        footNavFragment = new FootNavEnFragment();
        Bundle bundle = new Bundle();
        bundle.putInt("replace_id", R.id.framen_main_con);
        getSupportFragmentManager().beginTransaction()
                .replace(R.id.framen_foot_tab, footNavFragment).commitAllowingStateLoss();

        String countStr = UserDataPreference.GetUserData(MainEnActivity.this, UserDataPreference.NewChatmsgCount, "0");
...
//            if (userid != null && userid.length() > 0) {
            LocalInitDataLeft();//初始化本地数据
            if (pagenow == PageState.WODESHEBEI) {
...
//                        ShowDevicePage(PageState.DEVICECHANGE, myDeviceList.get(0).getMac());
...
//                            .replace(R.id.framen_main_con, new CChatFragment()).commitAllowingStateLoss();
        ...
        ShowDevicePage(index, mac);
    }

    public void ShowDevicePage(int index, String mac) {
...
//                                        .commit();
        ...
        getSupportFragmentManager().beginTransaction()
                .replace(R.id.framen_main_con_en, new MainConEnFragment())
                .commitAllowingStateLoss();
//        footNavFragment.Show(PageState.WODESHEBEI);
...
//                        ShowDevicePage(PageState.DEVICECHANGE, oznerDevices[0].Address());
                    } else {
                        ShowContent(PageState.WODESHEBEI, null);
//                        ShowDevicePage(PageState.WODESHEBEI, null);
...
//                    ShowDevicePage(PageState.DEVICECHANGE, mac);
                    myOverlayDrawer.toggleMenu();
                }
                break;
        }
...
//                        new ChangeDeviceThread(sendoraddress, null).start();
                        ...
                        if (MainEnActivity.this.isResume()) {
                            Log.e("Csir", " Resume AddDevice");
                            ShowContent(PageState.DEVICECHANGE, sendoraddress);
//                            ShowDevicePage(PageState.DEVICECHANGE, sendoraddress);
                        } else {
                            Log.e("Csir", "Not Resume AddDevice");
                            MainEnActivity.this.MAC = sendoraddress;
                        }
                        break;
                    case OznerBroadcastAction.ReceiveMessage:
                        ...
                        if (PageState.ZIXUNYEMIAN == pagenow) {
                            ...
                            if (UserDataPreference.GetUserData(MainEnActivity.this, "IsResume", "false").equals("false")) {
...
//                            ShowDevicePage(PageState.ZIXUNYEMIAN, null);
                        }
                        break;
...
//                        if (!loginToken.equals(OznerPreference.UserToken(MainActivity.this))) {
                        ...
                        if (localUserid != null
                                ...
                                && !loginToken.equals(OznerPreference.UserToken(MainEnActivity.this))) {
                            Intent loginIntent = new Intent();
//                            if (((OznerApplication) getApplication()).isLanguageCN()) {
                                loginIntent.setClass(getBaseContext(), LoginActivity.class);
//                            } else {
//                                loginIntent.setClass(getBaseContext(), LoginEnActivity.class);
//                            }
    ...
    public void showMsgNotification(String recvMsg, int notifyId) {
        try {
            ...
            if (msgObj.has("title")) {
                ...
                NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(MainEnActivity.this);
                ...
                if (UserDataPreference.GetUserData(this, "IsResume", "false").equals("false")) {//APP后台运行
                    Log.e("tag", "通知——后台启动");
                    Intent mainIntent = new Intent(this, MainEnActivity.class);
                    mainIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
//                    mainIntent.putExtra("isShowChat", "true");
                    UserDataPreference.SetUserData(MainEnActivity.this, "isShowChat", "true");
                    pendingIntent = PendingIntent.getActivity(MainEnActivity.this, 0x322, mainIntent, PendingIntent.FLAG_UPDATE_CURRENT);
                } else {
                    Log.e("tag", "通知——前台启动");
                    Intent broadIntent = new Intent(OznerBroadcastAction.ReceiveMessageClick);
                    broadIntent.putExtra("Address", "this is a fuck parms");
                    pendingIntent = PendingIntent.getBroadcast(MainEnActivity.this, 0x321
